    ...
    for (int i = points.size()-1; i > 2; i--) {
      ...
      if (points.get(i).groupID>0) { // black points have groupIndex=255
        ...
        if (selectedGroup!=1 && selectedGroup == points.get(i).groupID) {
          num(x1,y1,points.get(i).groupID);
        }
...
public void dotted() {
  ...
  n=PApplet.parseInt(l/(0.0001f+(points.get(current).sizer*5)));
  for (float k = 0; k<n;k++) {  
...
public void texter(int xx, int yy){
  textFont(font);
  textAlign(CENTER, CENTER);
  String ha = "potatoes mashed";
  fill(colorizer());
  int l1 = PApplet.parseInt(sqrt((xx-x1)*(xx-x1)+(yy-y1)*(yy-y1)));
  int l2 = PApplet.parseInt(sqrt((xx-x2)*(xx-x2)+(yy-y2)*(yy-y2)));
  pushMatrix();
  translate(xx, yy);
  rotate(atan2(y1-y2, x1-x2));
  if(distance<0.5f){
...
public void num(int xx, int yy, int n) {
  textFont(font);
  textAlign(CENTER, CENTER);
  fill(0,0,100);
  pushMatrix();
 ...
 */
    ...
    static public void main(String args[]) {
        PApplet.main(new String[] { "--bgcolor=#ECE9D8", "classed_mapper_osc" });
    }
}
